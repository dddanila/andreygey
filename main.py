...
def start_bot():
    ...
    def handler_call(call):
        ...
        if (call.data == "check_withdraw"):
            withdraw = CrystalPay.check_withdraw(str(withdraw_id))
            print(str(withdraw))
        ...
        if call.data == "pay_enter":
            bot.edit_message_text(chat_id=chat_id,
                                  message_id=message_id,
                                  text="Выберите способ оплаты:",
                                  reply_markup=menu.pay_enter
            )

        if call.data == 'replenish_balance':

            withdraw = func.replenish_balance(chat_id)
            ...
            replenish_balance.add(
                types.InlineKeyboardButton(text='💳 Оплатить', url=f"https://qiwi.com/payment/form/99?extra%5B%27account%27%5D={settings.QIWI_NUMBER}&amountInteger=None&amountFraction=0&extra%5B%27comment%27%5D={withdraw[1]}&currency=643&blocked[0]=account&blocked[2]=comment")
            )
